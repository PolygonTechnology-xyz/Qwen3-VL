...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        # Convert input_ids tensor to flat list for decode
        input_ids_raw = data_dict["input_ids"]
        if isinstance(input_ids_raw, torch.Tensor):
            # input_ids is shape [1, seq_len], convert to 1D list
            input_ids_list = input_ids_raw[0].tolist() if input_ids_raw.dim() >= 2 else input_ids_raw.tolist()
        else:
            input_ids_list = input_ids_raw
        
        # Ensure it's a flat list
        if isinstance(input_ids_list, list) and len(input_ids_list) > 0 and isinstance(input_ids_list[0], (list, tuple)):
        ...
        # Process labels: replace -100 with pad token
        labels_processed = [
            tid if tid != -100 else self.processor.tokenizer.pad_token_id
            for tid in labels
        ]
        
        label = self.processor.tokenizer.decode(labels_processed, skip_special_tokens=False)
