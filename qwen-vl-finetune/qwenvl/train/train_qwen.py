...
def preprocess_logits_for_metrics(logits, labels):
    """
    Preprocess logits to convert them to predicted token IDs.
    This is called by Trainer before passing to compute_metrics.
    ...
    Returns:
        Predicted token IDs of shape [batch, seq_len]
    """
    import torch
    import numpy as np
...
def create_compute_metrics_fn(processor):
    ...
    def compute_metrics(eval_preds):
        ...
        for pred, label in zip(predictions, labels):
            ...
            pred_ids = [int(p) for p in pred_ids if p != -100]
            
            # Replace -100 (ignored tokens) with pad token
            label_ids = [int(l) if l != -100 else processor.tokenizer.pad_token_id for l in label_ids]
