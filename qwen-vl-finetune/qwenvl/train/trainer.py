...
def flash_attention_forward(
    ...
    **kwargs,
    ...
    # Detect if attention_mask is cu_seqlens (1D, used in packed/data_flatten training)
    # or a standard mask (2D/4D, used during generation/inference)
    is_packed_format = (
    ...
    if not is_packed_format:
        # Fallback to SDPA for generation / standard inference
        # query/key/value are in shape: batch, head, seq_len, dim
        import torch.nn.functional as F
        
        if scaling is not None:
            query = query * scaling
        
        # Handle GQA: expand key/value heads to match query heads
        num_query_heads = query.shape[1]
        num_kv_heads = key.shape[1]
        if num_query_heads != num_kv_heads:
            num_groups = num_query_heads // num_kv_heads
            bsz, _, seq_len, head_dim = key.shape
            # Expand key and value: (batch, kv_heads, seq, dim) -> (batch, q_heads, seq, dim)
            key = key.unsqueeze(2).expand(bsz, num_kv_heads, num_groups, seq_len, head_dim).reshape(
                bsz, num_query_heads, seq_len, head_dim
            )
            bsz_v, _, seq_len_v, head_dim_v = value.shape
            value = value.unsqueeze(2).expand(bsz_v, num_kv_heads, num_groups, seq_len_v, head_dim_v).reshape(
                bsz_v, num_query_heads, seq_len_v, head_dim_v
            )
        
        # SDPA requires attn_mask to be bool, float, or match query dtype
        sdpa_mask = None
        ...
        # SDPA returns batch, head, seq_len, dim - transpose to batch, seq_len, head, dim
        attn_output = attn_output.transpose(1, 2)
        return attn_output, None
    
    # --- Packed flash attention path (training with data_flatten=True) ---
